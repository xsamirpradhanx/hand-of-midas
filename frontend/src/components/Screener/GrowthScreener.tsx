...
interface GrowthScreenerResult {
  symbol: string;
  price: number;
...
const GrowthScreener: React.FC = () => {
  const navigate = useNavigate();
  const [results, setResults] = useState<GrowthScreenerResult[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const { results: data, computedAt: at } = await api.getGrowthScreener();
      setResults(Array.isArray(data) ? data : []);
      setComputedAt(at);
      return at;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Failed to load growth screener';
      setError(msg);
      return null;
    } finally {
      setLoading(false);
  ...
  const handleRefreshScan = useCallback(async () => {
    ...
    setError(null);
    try {
      await api.refreshGrowthScreener();
    } catch (err: unknown) {
  ...
  return (
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>Growth Scanner</h1>
          <p className={styles.subtitle}>
            GARP — growth at a reasonable price: strong revenue/earnings growth paired with a PEG ratio that isn't stretched
          </p>
        </div>
        ...
        <div className={dashStyles.headerActions}>
          <div className={dashStyles.modeToggle} role="tablist" aria-label="Market session">
            ...
            <button type="button" role="tab" aria-selected={true} className={`${dashStyles.modeBtn} ${styles.modeBtnActiveGrowth}`} onClick={() => {}}>
              🚀 Growth
            </button>
      ...
      <div className={styles.methodBar}>
        <span className={styles.methodChip}>Revenue Growth &gt; 0%</span>
        <span className={styles.methodSep}>+</span>
        <span className={styles.methodChip}>PEG Ratio &gt; 0 (target ≤ 1.5)</span>
        <span className={styles.methodSep}>→</span>
        <span className={styles.methodResult}>Strong growth without paying any price for it</span>
      </div>
          ...
          <table className={styles.table}>
            <thead className={styles.thead}>
              <tr>
                ...
                <th className={styles.th}>Revenue Growth</th>
                <th className={styles.th}>Earnings Growth</th>
                <th className={styles.th}>PEG Ratio</th>
                <th className={styles.th}>Forward P/E</th>
            ...
            <tbody className={styles.tbody}>
                  ...
                  >
                    ...
                    <td className={styles.td}>
                      {r.revenueGrowth !== null ? (
                        <span className={`${styles.metricPill} ${r.revenueGrowth >= 0.25 ? styles.metricGood : ''}`}>
                          {(r.revenueGrowth * 100).toFixed(0)}%
                        </span>
                      ) : <span className={styles.naText}>—</span>}
                    </td>

                    <td className={styles.td}>
                      {r.earningsGrowth !== null ? (
                        <span className={`${styles.metricPill} ${r.earningsGrowth >= 0.20 ? styles.metricGood : ''}`}>
                          {(r.earningsGrowth * 100).toFixed(0)}%
                        </span>
                      ) : <span className={styles.naText}>—</span>}
                    </td>
                    ...
                    <td className={styles.td}>
                      {r.pegRatio !== null ? (
                        <span className={`${styles.metricPill} ${r.pegRatio <= 1.5 ? styles.metricGood : r.pegRatio > 2.0 ? styles.metricPoor : ''}`}>
                    ...
                    <td className={styles.td}>
                      {r.forwardPE !== null ? <span className={styles.metricPill}>{r.forwardPE.toFixed(1)}</span> : <span className={styles.naText}>—</span>}
                    </td>
              ...
              {results.length === 0 && !loading && (
                <tr className={styles.emptyRow}>
                  <td colSpan={7}>
                    <p className={styles.emptyTitle}>No growth setups found</p>
                    <p className={styles.emptyHint}>
                      No names currently combine positive growth with a reasonable PEG ratio. Try refreshing, or check back after the next daily scan.
                    </p>
                  </td>
                </tr>
              )}
